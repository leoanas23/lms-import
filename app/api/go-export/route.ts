...
export async function GET() {
  ...
  if (!meta) return NextResponse.json({ cached: false, storageWarning: storageIsEphemeral });
  const ageDays = Math.floor((Date.now() - new Date(meta.modified).getTime()) / 86400000);
  return NextResponse.json({ cached: true, ...meta, ageDays, stale: ageDays > 14, storageWarning: storageIsEphemeral });
}
